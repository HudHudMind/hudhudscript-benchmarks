-- LuaJIT (Lua 5.1) uyarlaması: LCG çarpımı (seed*1103515245) 2^53'ü aştığı için
-- mulmod (double-and-add) ile kesin hesaplanır; diğer kod orijinalle aynı.
-- Orijinal: benchmarks/src/lua/kmp_search.lua (dokunulmaz)
local seed = 42
local function rng()
    local a, b, r = seed, 1103515245, 0
    while b > 0 do
        if b % 2 == 1 then r = r + a; if r >= 2147483648 then r = r - 2147483648 end end
        a = a + a; if a >= 2147483648 then a = a - 2147483648 end
        b = math.floor(b / 2)
    end
    seed = (r + 12345) % 2147483648
