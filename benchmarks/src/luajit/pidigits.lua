-- LuaJIT (Lua 5.1) uyarlaması: BASE 10^9 → 10^7 (double'da rem*BASE 2^53 altında kalsın diye);
-- algoritma ve çıktı aynı. Orijinal: benchmarks/src/lua/pidigits.lua (dokunulmaz)
...
local function to_str(a)
    ...
    while tmp.n > 1 or tmp.d[1] > 0 do
        ...
        parts[#parts + 1] = string.format("%07d", rem)
    end
    local s = ""
